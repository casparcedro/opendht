...
void
DhtRunner::run(in_port_t port, const crypto::Identity identity, bool threaded, StatusCallback cb, bool is_bootstrap)
{
    sockaddr_in sin4;
    ...
    run(&sin4, &sin6, identity, threaded, cb, is_bootstrap);
}

void
DhtRunner::run(const char* ip4, const char* ip6, const char* service, const crypto::Identity identity, bool threaded, StatusCallback cb, bool is_bootstrap)
{
    auto res4 = getAddrInfo(ip4, service);
    auto res6 = getAddrInfo(ip6, service);
    run(res4.empty() ? nullptr : (sockaddr_in*) &res4.front().first,
        res6.empty() ? nullptr : (sockaddr_in6*)&res6.front().first, identity, threaded, cb, is_bootstrap);
}

void
DhtRunner::run(const sockaddr_in* local4, const sockaddr_in6* local6, const crypto::Identity identity, bool threaded, StatusCallback cb, bool is_bootstrap)
{
    if (running)
        return;
