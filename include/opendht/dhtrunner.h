...
public:
     ...
     */ 
    void run(in_port_t port, const crypto::Identity identity, bool threaded = false, StatusCallback cb = nullptr, bool is_bootstrap = false);
     ...
     * @param cb: Optional callback to receive general state information.
     */
    void run(const sockaddr_in* local4, const sockaddr_in6* local6, const crypto::Identity identity, bool threaded = false, StatusCallback cb = nullptr, bool is_bootstrap = false);

    /**
     * Same as @run(sockaddr_in, sockaddr_in6, Identity, bool, StatusCallback), but with string IP addresses and service (port).
     */
    void run(const char* ip4, const char* ip6, const char* service, const crypto::Identity identity, bool threaded = false, StatusCallback cb = nullptr, bool is_bootstrap = false);
